...
class SimpleHTTPRequestHandler(BaseHTTPServer.BaseHTTPRequestHandler):
    ...
    def list_directory(self, path):
        ...
        f.write('<HR size="1" noshade="noshade">')
        f.write('<table border=0 width="100%" cellspacing="0" cellpadding="5" align="center" style="overflow: scroll;word-break: keep-all"><tr bgcolor="#00DB00"><td>序号</td><td>文件名</td><td>文件大小</td><td>文件创建时间</td><td>MD5</td><td>操作</td></tr>')
        idn=1
        for name in list:
            ...
            f.write(
                ...
                % (idn,urllib.quote(linkname), ToDo().do_gbk2utf8(colorName),
                   ToDo().sizeof_fmt(filesize), ToDo().modification_date(filename),emd5,"?delete="+urllib.quote(linkname)))
            idn = idn+1
        list = []
        f.write('</table>\n<HR size="1" noshade="noshade">\n<h2>Powered By kanbuxiaqu@outlook.com</h2>\n</body>\n</html>\n')
        length = f.tell()
